...
import { QUESTION_NAMES } from "@/lib/questions";
...
export default function HormoneResultPage() {
    const router = useRouter();
    const [result, setResult] = useState<StoredResult | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const data = getResult("hormone");
        if (!data) {
            router.replace("/tools/hormone");
            return;
        }
        setResult(data);
        setLoading(false);
    ...
    const analysis = analyzeScore(result.answers, QUESTION_NAMES);
    ...
    const chartData = Object.entries(result.answers).map(([key, value]) => ({
        subject: QUESTION_NAMES[key] || key,
        A: value,
        fullMark: 3,
    }));
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        ...
                        {recommendedIngredients.length === 0 && (
                            <div className="col-span-3 text-center text-gray-500 py-8 bg-gray-50 rounded-xl">
                                현재 특별히 권장되는 성분이 없습니다. 균형 잡힌 식단을 유지하세요.
                            </div>
                        )}
                    </div>
                </section>
