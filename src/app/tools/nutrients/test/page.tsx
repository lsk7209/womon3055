...
export default function NutrientsPage() {
    ...
    const handleAnswer = (value: number) => {
        const questionId = NUTRIENT_QUESTIONS[currentStep].id;
        setAnswers((prev) => ({
            ...prev,
            [questionId]: value,
        }));
    ...
    const handleComplete = () => {
        ...
        const persona = classifyPersona(analysis.totalScore); // 영양소는 페르소나가 덜 중요하지만 통일성을 위해 유지
            ...
            <div className="max-w-3xl mx-auto">
                <div className="text-center mb-12">
                    <h1 className="text-3xl font-bold text-gray-900 mb-4">
                        {TOOL_METADATA.nutrients.title}
                    </h1>
                    <p className="text-lg text-gray-600">
                        {TOOL_METADATA.nutrients.description}
                    </p>
                </div>

                <div className="bg-white rounded-2xl shadow-xl p-6 md:p-10">
                    <StepWizard
                        totalSteps={NUTRIENT_QUESTIONS.length}
