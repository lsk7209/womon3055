...
import { getPersona, getPersonaChartColor, getPersonaColor } from "@/lib/persona";
import { analyzeScore, generateInsights, generateRecommendations } from "@/lib/scoring";
import { NUTRIENT_QUESTIONS } from "@/lib/questions";
...
export default function NutrientsResultPage() {
    const router = useRouter();
    const [result, setResult] = useState<StoredResult | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const data = getResult("nutrients");
        if (!data) {
            router.replace("/tools/nutrients");
            return;
        }
        setResult(data);
        setLoading(false);
    ...
    const questionNames = NUTRIENT_QUESTIONS.reduce((acc, q) => {
        acc[q.id] = q.text;
        return acc;
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        ...
                        {recommendedIngredients.length === 0 && (
                            <div className="col-span-3 text-center text-gray-500 py-8 bg-gray-50 rounded-xl">
                                현재 특별히 결핍이 의심되는 영양소가 없습니다. 균형 잡힌 식단을 유지하세요.
                            </div>
                        )}
                    </div>
                </section>
